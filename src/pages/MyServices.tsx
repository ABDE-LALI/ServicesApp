...
import { Eye, Star, Phone, Mail, Link as LinkIcon, Plus, X, MapPin } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import 'leaflet/dist/leaflet.css';

const LocationPicker = ({ onLocationSelect }: { onLocationSelect: (location: [number, number]) => void }) => {
  useMapEvents({
    click(e) {
  ...
  const handleAddService = async (e) => {
    ...
    try {
      ...
      const { data: categoryData, error: categoryError } = await supabase
        .from('categories')
        .select('name_en')
        .eq('id', newService.category_id)
        .single();

      if (categoryError) throw categoryError;

      // Create the service with the correct image URL
      const categoryName = categoryData.name_en.toLowerCase().replace(/\s+/g, '-');
      const imageUrl = `/categories/${categoryName}.jpg`;
      ...
      const { error: providerServiceError } = await supabase
        .from('provider_services')
        .insert({
          provider_id: user.id,
          ...
          contact_website: newService.website || null,
        });
