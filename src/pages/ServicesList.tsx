...
export const ServicesList = () => {
  ...
  const fetchCategories = async () => {
    try {
      const { data, error } = await supabase.from("categories").select("id, name_en, name_ar, name_fr")
  ...
  const fetchServices = async (categoryId?: string | null) => {
    try {
      setIsLoading(true)
      setError(null)
  ...
  const handleServiceClick = (providerId: string) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        >
          <h2 className="text-xl font-bold mb-4 text-neutral">Categories</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {services.map((service, index) => (
                <motion.div
                  ...
                  onClick={() => service.providers?.[0] && handleServiceClick(service.providers[0].id)}
                >
                    ...
                    {service.providers?.map((provider) => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-semibold text-neutral">{provider.provider.username}</span>
                          <div className="flex items-center space-x-1">
                            <Star className="w-4 h-4 text-yellow-500" />
                            <span className="text-sm text-gray-600">{provider.rating_avg.toFixed(1)}</span>
                          </div>
                        </div>
